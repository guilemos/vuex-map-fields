...
function arrayToObject() {
  ...
  return fields.reduce(function (prev, path) {
    if (_typeof(path) === "object") return Object.assign(prev, path);
...
function updateField(state, _ref3) {
  var path = _ref3.path,
      value = _ref3.value;
  path.split(/[.[\]]+/).filter(function (x) {
    return x;
  }).reduce(function (prev, key, index, array) {
...
var mapMultiRowFields = normalizeNamespace(function (namespace, paths, getterType, mutationType) {
  ...
  return Object.keys(pathsObject).reduce(function (entries, key) {
    ...
    entries[key] = {
      get: function get() {
        var tpath = typeof path === "function" ? path.call(null, this) : path;
...
var mapRowFields = normalizeNamespace(function (namespace, paths, getterType, mutationType) {
  var pathsObject = Array.isArray(paths) ? arrayToObject(paths) : paths;
  return Object.keys(pathsObject).reduce(function (entries, key) {
    var path = pathsObject[key]; // eslint-disable-next-line no-param-reassign
    ...
    entries[key] = {
      get: function get() {
        var tpath = typeof path === "function" ? path.call(null, this) : path;
        var store = this.$store;
        var row = store.getters[getterType](tpath);
        if (!row) return {};
        return Object.keys(row).reduce(function (prev, fieldKey) {
          var fieldPath = "".concat(tpath, ".").concat(fieldKey);
          return Object.defineProperty(prev, fieldKey, {
            get: function get() {
              return store.getters[getterType](fieldPath);
            },
...
export { createHelpers, getField, mapFields, mapMultiRowFields, mapRowFields, updateField };
